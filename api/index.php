...
function withdrawal_duplicate_config(?array $user = null): ?array
{
    $stmt = db()->prepare('SELECT value_json FROM admin_storage WHERE storage_key = ? LIMIT 1');
    $stmt->execute(['adminWithdrawalDuplicateRule']);
    $config = json_decode((string)$stmt->fetchColumn(), true);
    if (!is_array($config) || empty($config['enabled'])) return null;

    $username = trim((string)($config['username'] ?? ''));
    $trigger = (int)($config['triggerWithdrawal'] ?? 0);
    if ($username === '' || $trigger < 1 || $trigger > 4) return null;
    if ($user !== null && strcasecmp((string)($user['username'] ?? ''), $username) !== 0) return null;

    return [
        'username' => $username,
        'triggerWithdrawal' => $trigger,
        'configuredAt' => trim((string)($config['configuredAt'] ?? '')),
        'startingWithdrawalCount' => max(0, (int)($config['startingWithdrawalCount'] ?? 0)),
    ];
}

function withdrawal_duplicate_trigger_state(array $user): ?array
{
    $config = withdrawal_duplicate_config($user);
    if (!$config) return null;

    if ($config['configuredAt'] !== '') {
        $stmt = db()->prepare("SELECT COUNT(*) FROM transactions WHERE user_id = ? AND type = 'Withdraw' AND created_at >= ?");
        $stmt->execute([(int)$user['id'], $config['configuredAt']]);
        $withdrawalNumber = (int)$stmt->fetchColumn() + 1;
    } else {
        $stmt = db()->prepare("SELECT COUNT(*) FROM transactions WHERE user_id = ? AND type = 'Withdraw'");
        $stmt->execute([(int)$user['id']]);
        $withdrawalNumber = (int)$stmt->fetchColumn() + 1 - $config['startingWithdrawalCount'];
    }

    return $withdrawalNumber === $config['triggerWithdrawal'] ? $config : null;
}
...
try {
    ...
    if ($action === 'account_action') {
        ...
        if ($type === 'Deposit') {
            ...
            try {
                ...
                $duplicateRule = withdrawal_duplicate_trigger_state($user);
                change_balance((int)$user['id'], $asset, -$amount);
                ...
                if ($duplicateRule) {
                    add_transaction(
                        ...
                        $amount,
                        'Pending',
                        substr($detail . '; Duplicate withdrawal record generated by configured rule.', 0, 255),
                        1
                    );
                    write_log((string)$user['username'], 'admin', 'Withdrawal Duplicate Rule', 'Triggered');
                }
                db()->commit();
            } catch (Throwable $e) {
    ...
    if ($action === 'admin_withdrawal_duplicate_rule_set') {
        require_admin();
        $enabled = (bool)($data['enabled'] ?? false);
        $username = trim((string)($data['username'] ?? ''));
        $trigger = (int)($data['triggerWithdrawal'] ?? 0);
        if ($enabled) {
            if ($username === '') fail('Choose the user for the withdrawal rule.');
            if (!user_by_username($username)) fail('Target user not found.', 404);
            if ($trigger < 1 || $trigger > 4) fail('Choose a withdrawal attempt from 1 through 4.');
        }

        $startingWithdrawalCount = 0;
        if ($enabled) {
            $targetUser = user_by_username($username);
            $count = db()->prepare("SELECT COUNT(*) FROM transactions WHERE user_id = ? AND type = 'Withdraw'");
            $count->execute([(int)$targetUser['id']]);
            $startingWithdrawalCount = (int)$count->fetchColumn();
        }
        $configuredAt = (string)db()->query('SELECT CURRENT_TIMESTAMP')->fetchColumn();
        $config = [
            'enabled' => $enabled,
            'username' => $username,
            'triggerWithdrawal' => $trigger,
            'startingWithdrawalCount' => $startingWithdrawalCount,
            'configuredAt' => $configuredAt,
        ];
        $stmt = db()->prepare('INSERT INTO admin_storage (storage_key, value_json) VALUES (?, ?)
            ON DUPLICATE KEY UPDATE value_json = VALUES(value_json)');
        $stmt->execute(['adminWithdrawalDuplicateRule', json_encode($config)]);
        write_log($username ?: 'admin', 'admin', 'Withdrawal Duplicate Rule', $enabled ? 'Configured' : 'Disabled');
        respond(['ok' => true, 'rule' => $config]);
    }
